...
async def send_email_change_confirmation_email(to_email: str, new_email: str, token: str) -> None:
    confirm_url = _build_app_url(f"/api/auth/confirm-email-change?token={token}")
    html = f"""
    <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;">
        <h2 style="color:#2563eb;">Подтверждение смены email</h2>
        <p>Вы запросили смену email в системе <strong>ЖКХ Диспетчерская</strong>.</p>
        <p>Новый email: <strong>{new_email}</strong></p>
        <p>Если это действительно вы, подтвердите изменение по кнопке ниже:</p>
    ...
    message = MessageSchema(
        subject="Подтверждение смены email — ЖКХ Диспетчерская",
        recipients=[to_email],
        body=html,
        subtype=MessageType.html
    )
