...
async def create_message(
    ...
    return await create_message_service(db, ticket_id, payload.text, current_user)
...
@router.post("/{ticket_id}/files/upload", response_model=TicketFileOut, status_code=status.HTTP_201_CREATED)
async def upload_ticket_file(
    ...
    return await upload_ticket_file_service(db, ticket_id, file, current_user)
...
@router.post(
    "/{ticket_id}/messages/{message_id}/files/upload",
